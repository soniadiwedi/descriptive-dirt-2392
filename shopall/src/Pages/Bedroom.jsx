...
export default function Bedroom() {
    ...
    async function fetchData(page){
        try{
            let res= await fetch(`https://639b295bd5141501974d54b3.mockapi.io/bedrooms?page=${page}&limit=6`)
            let data=await res.json()
            setdata(data)
            setSearchApi(data)
...
/////////////////////////////////////////////////////////
    function handleSearch(e){
        if(e.target.value==""){
          ...
          const result =  searchApi.filter(item=>item.category.toLowerCase().includes(e.target.value.toLowerCase()))
          setdata(result)
        }
    ...
    return<>
        <Box margin="auto" width="30%"> 
            <Input placeholder="Search" value={filter} onChange={(e)=>handleSearch(e)} />
        </Box>
    ...
    <Box marginLeft="40%" justifyContent="center"  >
        <Pagination total={total} settotal={settotal} page={page} setpage={setpage}  noofbtn={noofbtn} />
        </Box>
